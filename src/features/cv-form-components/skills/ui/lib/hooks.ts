...
export const useSaveSkill = (): SaveSkill => {
  ...
  const onSaveSkillHandle = useCallback(
    async (skill: Skill, user: DbUser): Promise<void> => {
...
export const useDeleteSkill = (): IUseDeleteSkill => {
  const isLastSkillGroup = (id: string, projects: Project[]): boolean => {
    const isLast =
      projects
        .filter((project) => project.categories.some((category) => category.split(", ")[0] === id))
        .some((project) => project.categories.length === 1) || false

    return isLast
  }

  const isLastSkill = (id: string, projects: Project[]): boolean | undefined => {
    const isLast = projects
      .filter((project) => project.categories.some((category) => category.includes(id)))
      .some((project) => project.categories.some((category) => category.split(", ")[1].split(",").length === 1))

    return isLast
  }

  const removeProjectSkillById = (skillId: string, projects: Project[]): Project[] =>
    projects.map((userProject) => {
      const filteredCategories = userProject.categories.map((category) => {
        if (category.includes(skillId)) {
          const filteredCategory = category
            .split(", ")[1]
            .split(",")
            .filter((categoryItem) => categoryItem !== skillId)
  ...
  const removeProjectSkillGroupById = (skillGroupId: string, projects: Project[]): Project[] =>
    projects.map((userProject) => {
      const filteredCategories = userProject.categories.filter((category) => !category.includes(skillGroupId))
