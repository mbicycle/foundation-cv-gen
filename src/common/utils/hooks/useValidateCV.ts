...
export const useValidateCV = (): Validity => {
  ...
  const { title = "", summary = "", languages = [], skills = [], projects = [] } = data ?? {}
  ...
  const isInvalidSkills =
    !skills.length ||
    skills.some((skill) => skill.name?.trim().length < 2 || skill.tools?.some((tool) => tool.name?.trim().length < 2))
  const isInvalidProjects =
    !projects.length ||
    projects.some(
      (project) =>
        project.title.length < 2 || project.role.length < 2 || project.teamSize <= 0 || project.description.length < 2,
    )
