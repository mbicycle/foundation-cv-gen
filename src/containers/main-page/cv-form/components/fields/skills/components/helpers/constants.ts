...
export const getSkillSchema = (skillGroupNames: string[]): yup.InferType<yup.AnySchema> => yup.object({
  name: yup.string().lowercase().min(2, ErrorMessage.MinimumTwo).trim()
    ...
    yup.object().shape({
      ...
      experience: yup.number()
        ...
        .test(
          ...
          (value) => (value ? value % 0.5 === 0 : true),
        ),
    }),
  ).min(1),
}).required();

export const getSkillGroupNames = (skillGroups: Skill[], id: string): string[] => skillGroups
  .filter((skillGroup) => skillGroup.id !== id)
  .map((userSkill) => userSkill.name.toLowerCase());
