    ...
    for (let i = startPage; i <= endPage; i += 1) {
      ...
      if (i === startPage) {
        doc.roundedRect(
          ...
          borderY,
          191,
          PAGE_HEIGHT_FOR_BORDER - borderY,
          1,
          1,
        );
...
export async function drawUserMailAndSkype(
  props: {
  doc: JsPDF,
  pageWidth: number,
  isHidden: boolean,
  ...
  const {
    isHidden, doc, pageWidth, email, telegram, skype,
  } = props;
  if (isHidden) return;
  ...
  const startX = pageWidth - LEFT_RIGHT_PADDING / 1.5
      - Math.max(mailNameWidth, telegramNameWidth, skypeNameWidth);
...
export async function drawPersonalInformation(
  props: {
      doc: JsPDF,
      name: string,
      isHidden: boolean,
  ...
  const {
    doc, name, title, summary, projects, photo, isHidden,
  } = props;
  y = 26;
  infoY = y;
  ...
  doc.roundedRect(
    LEFT_RIGHT_PADDING,
    y,
    PAGE_WIDTH - (LEFT_RIGHT_PADDING * 2),
    44,
    1,
    1,
    'F',
        ...
        skill.tools.forEach((tool) => {
          ...
          regions.push({
            callback: (): void => {
              ...
              doc.text(
                tool?.level || '',
                (PAGE_WIDTH / 3) * 2 + ((PAGE_WIDTH / 3) / 3) - 20,
                y + 6,
              );
      ...
      regions.push({
        callback: (): void => {
          ...
          if (!dayjs(from).isSame(dayjs(to), 'month')) {
            const toDays = to === projectDatePresent
              ? to
              : dayjs(to || new Date().toString()).format('MMMM YYYY');
            fromTo += ` - ${toDays}`;
          }
