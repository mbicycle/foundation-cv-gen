...
const createInlineTextRun = (titleText: string, text: string): Paragraph => new Paragraph({
  indent: INDENT,
  children: [
    new TextRun({
      text: titleText,
...
const createResponsibilities = (titleText: string, responsibilities: string[]): Paragraph => new Paragraph({
  indent: INDENT,
  children: [
    new TextRun({
      text: titleText,
...
export const createProjects = (projects: Project[] | undefined, isHidden: boolean): Paragraph[] => {
  ...
  if (projects?.length) {
    projects.forEach((project) => {
      const {
        title, description, from, to, role, categories, responsibilities, link, teamSize,
      } = project;
      ...
      paragraphArr.push(createInlineTextRun(
        'Involvement duration: ',
        `${dayjs(from).format('MMMM YYYY')}${dayjs(from).isSame(dayjs(to), 'month')
...
export const createWordProjects = (
  { projects, isHidden }: { projects: Project[] | undefined; isHidden: boolean; },
): Paragraph[] => createProjects(projects, isHidden);

const useWordProjects = (
  { projects }: { projects: Project[] | undefined; },
): { wordProjects: Paragraph[]; } => {
