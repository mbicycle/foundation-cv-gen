...
const AdminTableProvider: React.FC<React.PropsWithChildren> = function ({ children }): JSX.Element {
  const [wildcardState, wildcardDispatch] = useReducer(wildcardReducer, { wildcard: [], column: "displayName" })
  ...
  return (
    <TableDataContext.Provider value={tableDataContextValue}>
      <WildcardContext.Provider value={wildcardContextValue}>{children}</WildcardContext.Provider>
